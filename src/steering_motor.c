...
void set_servo_angle(int pulse_width) {
    ...
    with 0.5ms as 90° left, 1.5ms as 0°, and 2.5ms as 90° right. */
    ...
    uint slice_num = pwm_gpio_to_slice_num(STEERING_MOTOR_PIN);
    /* Pulse width (in us) is converted to PWM level. As PWM wrap value is 39062, full 20ms period is
...
void steer_right(int angle) {
    /* Note that angle is in 10° increments (e.g. 0 = 0°, 2 = 20°), up to 9 (90°). Each step corresponds
    to an increase of approx 111us in pulse width. */ 
    int pulse_width;

    if (angle >= 0 && angle <= 9) {
        // 4 added so that result is effectively rounded even though it is being truncated.
        pulse_width = 1500 + ((angle * 1000 + 4) / 9);
...
void steer_left(int angle) {
    /* Note that angle is in 10° increments (e.g. 0 = 0°, 2 = 20°), up to 9 (90°). Each step corresponds
    to a decrease of approx 111us in pulse width. */  
    int pulse_width;
